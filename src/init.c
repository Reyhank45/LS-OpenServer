...
int main(void) {
  ...
  /*
   * Boot phase complete. From here on, /dev/console belongs to the
   * shell. Redirect init's own stderr to /dev/null so any stray
   * fprintf(stderr,...) calls (e.g. from child execve failures)
   * don't corrupt the terminal. All logging now goes to logfp only.
   */
  boot_done = 1;
  {
